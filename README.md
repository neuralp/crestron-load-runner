...
The C runtime is linked into the executable (`.cargo/config.toml`), so the one file runs on a PC without the Visual C++ redistributable.

Versions are written `YY.M.P` — the year's last two digits, then the month and a patch number — because the version is also the installer's, and Windows Installer allows at most 255 in the first place. Builds before 26.9.11 wrote the year in full, so 2026.9.10 is the same release as 26.9.10.
...
This builds the release executable and wraps it in `target\installer\CrestronLoadRunner-<version>.msi`, using [WiX 5](https://wixtoolset.org/) (`dotnet tool install --global wix`). The definition is `installer/crestron-load-runner.wxs`.

The installer needs no administrator rights: it installs for the current user into `%LOCALAPPDATA%\Programs\Crestron Load Runner` and adds a Start-menu shortcut. There are no pages to click through. Installing a newer version replaces the older one in place, and installing an older one over a newer is refused.

Uninstalling, from **Settings → Apps**, removes the program and the shortcut, and then asks what else to remove. There is one checkbox each for the preferences, the firmware library, the script library, and the passwords and API tokens saved in Windows Credential Manager. Nothing is ticked to begin with, so anything left alone is there again after a reinstall. Folders the application created are removed once they are empty. An upgrade never asks, and nor does a silent uninstall (`msiexec /x … /qn`), which keeps everything. The same choice can be made at any time with `crestron-load-runner --remove-data`.

The installer is not signed, so the first time it is opened Windows SmartScreen may warn that it comes from an unknown publisher; choose **More info → Run anyway**.
...
- `--remove-data`: open the window the uninstaller shows, which offers to remove the saved preferences, firmware library, scripts and passwords one kind at a time, and exit. With `--config-dir` it offers that profile's data instead, and removes no folder above it.
...
The editor offers the current address book's devices, plus any devices discovered this session, as assignment targets. Opening it does not scan the network. For devices the address book doesn't cover, click **Discover devices** in the window (or **Discover Devices** in the main window). The window shows discovery progress and adds results to its device choices as they arrive, without clearing assignments. Each choice is marked *address book* or *discovered*.
...
Choose whether peripherals should reference the **processor** by **IP address** or **HOSTNAME** and review the displayed master address. Select a device beside each CIP_ID/model row, or leave it at **Skip**. Any model can be assigned: a model mismatch shows a warning on that row but does not prevent GO. Rows are compared with the discovered model when available, otherwise the address book's model, ignoring case and surrounding whitespace. The same physical device cannot be selected twice. Address book devices that have not been discovered are reached at their configured host. Editable display labels are never treated as hostnames. Rediscover if the processor's address information is unavailable.
...
**Save token** keeps the API token in the system vault, keyed the same way as a remembered password (below), and it is used again whenever that server is opened from any book. Editing the field alone saves nothing. **Forget token** removes the saved token. For a discovered server, either button first adds it to the address book and saves the book, opening Save As for an untitled one. The approved HTTPS endpoint and certificate fingerprint stay in the address book. A book written by an earlier build may still contain tokens. Opening it moves them into the vault and marks the book as changed, so saving it removes them from the file. Tokens are masked in the UI, redacted in debug output, and not written to device logs or preferences. Fetched data remains session-only. Changing connection settings clears cached results; connect again to use the new settings.
...
The first connection presents the device's SHA-256 host-key fingerprint. Trust it only after comparing it with a known-good fingerprint. A changed key is rejected. Passwords and API tokens are never written to an address book or to `preferences.json`. The default password, per-device passwords with **Remember password** ticked (in Device details or **Add Device**), and saved VC-4 API tokens are kept in the operating system's per-user vault. That is **Windows Credential Manager** (under *Windows Credentials*, named after `CrestronLoadRunner`) on Windows, and the **Secret Service** (GNOME Keyring or KWallet; see `seahorse`, or `secret-tool search service CrestronLoadRunner`) on Linux. Both are unlocked by signing in, so there is no master password. They are fetched automatically when a connection needs them. A remembered password is keyed by the device's MAC address, so it follows the device through a new IP address from DHCP and is found whether the device is reached by hostname or by address. A device whose MAC is not known yet, such as one added by hand and not yet seen by discovery, is keyed by host and port until discovery reports its MAC, and its password then moves to the MAC key. Replacing the hardware at an address means a new MAC, so the old unit's password is not sent to it. Every book that lists the device uses the same saved password, and removing a device from the list does not forget it: untick **Remember password** to do that. A `--config-dir` profile keeps its secrets under a name of its own, so it neither sees nor changes the real profile's.

When there is no vault — a Linux session without a Secret Service, such as a headless machine or a minimal window manager — a notice says so at startup. Saved passwords and tokens are then kept only until the application closes, and nothing is written to disk in their place.

Session credentials entered at the prompt are held in memory only — they are not written anywhere. Every password and token is redacted from debug output.
